...
**The average concentration of PM2.5 across all census tracts in California is 10.19529** 
...
**The estimated slope coefficient for PM2.5 is 0.118162. This means that for every one unit increase in PM2.5 levels, there is a 0.118162 percent increase in the percentage of low birth weights for the California census tract.**

**The standard error is 0.008334**

**The effect of PM2.5 on LowBirthWeight is statistically significant at the 5% level becuase our p-value is much lower than 0.05.**

```{r}
model_1 <- summary(lm_robust(Low.Birth.Weight ~ PM2.5, data = mydata))
...
**The estimated coefficient on Poverty is 0.02744. This means that for every one unit increase in poverty (the percent of population living below two times the federal poverty level), there is a 0.02744 percent increase in the percentage of low birth weights for the California census tract.** 

**The estimated coefficient on PM2.5 decreased to 0.05911 compared to 0.1182 in part (d).** 


```{r}
model_2 <- summary(lm_robust(Low.Birth.Weight ~ PM2.5 + Poverty, data = mydata))
...
```{r}
linearHypothesis(model_2,c("PM2.5=0", "Poverty=0"), white.adjust = "hc2")
```
